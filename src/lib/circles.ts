  ...
  for (let i = 0; i < txHashes.length; i += batchSize) {
    ...
    try {
      ...
      for (let j = 0; j < responses.length; j++) {
        ...
        const gameKeys = [...ALL_GAMES.map(g => g.key), "blackjack", "lootbox", "lottery", "coin_flip", "hilo", "mines", "dice", "crash_dash", "keno", "roulette", "plinko", "daily", "nf_auth", "nf_auth_v2", "nf_cashout", "wallet"];
        let found = false;
