...
export default function DamesLobbyPage() {
  ...
  async function handleCreate() {
    if (isDemo) {
      router.push(`/dames/DEMO-${Math.random().toString(36).slice(2, 8).toUpperCase()}`)
      return
    }
    setLoading(true)
    setError('')
    try {
      ...
      sessionStorage.setItem(`dames_creator_${data.id}`, '1')
      router.push(`/dames/${data.id}`)
    } catch {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-amber-500/10 mb-4 text-3xl">
            ♟️
          </div>
