...
export default async function MinesLobbyPage() {
  const [tables, realStakesEnabled] = await Promise.all([
    db
    .select()
    .from(minesTables)
    .where(eq(minesTables.status, "active")),
    isRealStakesEnabled(),
  ]);
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-2xl mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-ink mb-2">Mines</h1>
        <p className="text-sm text-ink/50 max-w-md mx-auto">
          Grille 5x5 — revelez les gemmes, evitez les mines. RTP ~99%.
        </p>
      </div>
