...
const DEFAULT_ROWS: RewardRow[] = [
  { id: "fragments-75", kind: "fragments", label: "+75 Fragments", probability: "45", fragmentsValue: "75", crcValue: "0", color: "#10B981" },
  { id: "fragments-200", kind: "fragments", label: "+200 Fragments", probability: "35", fragmentsValue: "200", crcValue: "0", color: "#38BDF8" },
  { id: "fragments-500", kind: "fragments", label: "+500 Fragments", probability: "17", fragmentsValue: "500", crcValue: "0", color: "#8B5CF6" },
  { id: "crc-1", kind: "crc", label: "+1 CRC", probability: "2.8", fragmentsValue: "0", crcValue: "1", color: "#F59E0B" },
  { id: "crc-10", kind: "crc", label: "+10 CRC", probability: "0.2", fragmentsValue: "0", crcValue: "10", color: "#EC4899" },
];
...
function kindFromReward(reward: ApiReward): RewardKind | null {
  ...
  if (Number(reward.fragmentsValue) > 0) return "fragments";
  return null;
}
...
function rowFromReward(reward: ApiReward, index: number): RewardRow | null {
  ...
  return {
    ...
    fragmentsValue: formatNumber(Number(reward.fragmentsValue ?? 0)),
...
function typeForRow(row: RewardRow, index: number): string {
  const fragments = Math.floor(parseNumber(row.fragmentsValue));
  const crc = parseNumber(row.crcValue);
  if (row.kind === "fragments") return `fragments_${sanitizeTypePart(fragments)}_${index + 1}`;
...
function labelForRow(row: RewardRow): string {
  ...
  if (row.kind === "fragments") return `+${Math.floor(parseNumber(row.fragmentsValue))} Fragments`;
  return `+${formatNumber(parseNumber(row.crcValue))} CRC`;
}

function buildPayload(rows: RewardRow[]): ApiReward[] {
  return rows.map((row, index) => {
    const fragmentsValue = row.kind === "fragments" ? Math.floor(parseNumber(row.fragmentsValue)) : 0;
...
function RewardLine({ title, play }: { title: string; play?: DailyPlayResult }) {
  ...
  return (
    <p className="font-bold text-ink dark:text-white">
      ...
      {Number(result?.fragmentsValue ?? 0) > 0 && (
        <span className="text-violet-600"> - +{result?.fragmentsValue} Fragments</span>
      )}
      {play?.payout?.error && (
...
export function DailyTab({ password }: { password: string }) {
  ...
  const summary = useMemo(() => {
    ...
    const expectedFragments = rows.reduce((sum, row) => (
      sum + (Math.max(0, parseNumber(row.probability)) / 100) * (row.kind === "fragments" ? Math.floor(parseNumber(row.fragmentsValue)) : 0)
    ), 0);
  ...
  function updateRow(id: string, patch: Partial<RewardRow>) {
    ...
    setRows((current) => current.map((row) => {
      ...
      if (patch.kind === "fragments") return { ...next, crcValue: "0" };
      if (patch.kind === "crc") return { ...next, fragmentsValue: "0" };
      return next;
    }));
  }
  ...
  function addRow(kind: RewardKind) {
    ...
    setRows((current) => [
      ...current,
      {
        id,
        kind,
        label: kind === "fragments" ? "+100 Fragments" : "+1 CRC",
        probability: "1",
        fragmentsValue: kind === "fragments" ? "100" : "0",
      ...
      <section className="rounded-xl border border-ink/10 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-zinc-950">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h3 className="mt-2 text-lg font-black text-ink dark:text-white">Roue daily Fragments / CRC</h3>
            <p className="mt-1 max-w-3xl text-sm font-semibold text-ink/65 dark:text-white/65">
              Configure les gains de la roue quotidienne. Chaque tirage donne soit des Fragments jouables, soit un gain CRC rare.
            </p>
            <p className="mt-2 text-xs font-bold text-ink/45 dark:text-white/45">
              Garde-fou Fragments: {formatNumber(MAX_DAILY_FRAGMENTS_REWARD)} max par ligne, {formatNumber(MAX_DAILY_EXPECTED_FRAGMENTS)} moyen max par daily.
            </p>
          </div>
          <button
            onClick={saveConfig}
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          />
          <StatBox label="Fragments moyens / daily" value={formatNumber(summary.expectedFragments)} accent="text-violet-600" />
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-3">
          <StatBox label="100 daily" value={`${formatNumber(summary.expectedFragments * 100)} Fragments / ${formatNumber(summary.expectedCrc * 100)} CRC`} />
          <StatBox label="1000 daily" value={`${formatNumber(summary.expectedFragments * 1000)} Fragments / ${formatNumber(summary.expectedCrc * 1000)} CRC`} />
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <button onClick={() => addRow("fragments")} className="inline-flex items-center gap-2 rounded-lg border border-ink/10 px-3 py-2 text-sm font-bold text-ink hover:bg-ink/5 dark:border-white/10 dark:text-white dark:hover:bg-white/5">
          ...
          <table className="min-w-[920px] w-full border-collapse text-sm">
            <thead className="bg-ink/[0.03] text-left text-[10px] font-black uppercase tracking-widest text-ink/50 dark:bg-white/[0.04] dark:text-white/50">
              <tr>
                ...
                <th className="px-3 py-3">Fragments</th>
              ...
              {loadingConfig ? (
                ...
                <tr key={row.id} className="border-t border-ink/10 dark:border-white/10">
                  ...
                  <td className="px-3 py-2">
                    <input
                      inputMode="numeric"
                      disabled={row.kind !== "fragments"}
                      value={row.fragmentsValue}
                      onChange={(event) => updateRow(row.id, { fragmentsValue: event.target.value })}
