...
export async function POST(req: NextRequest) {
  try {
    ...
    const [game] = await db.select().from(damesGames).where(eq(damesGames.slug, gameSlug)).limit(1);
    ...
    for (const payment of newPayments) {
      ...
      if (payment.gameData) {
        if (payment.gameData.game !== "dames" || payment.gameData.id !== game.slug) continue;
      } else {
        continue;
      }
    ...
    const [updated] = await db.select().from(damesGames).where(eq(damesGames.slug, gameSlug)).limit(1);
