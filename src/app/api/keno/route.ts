...
/**
 * GET /api/keno?slug=classic
 * Returns a keno table config.
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const [table] = await db.select().from(kenoTables).where(eq(kenoTables.slug, slug)).limit(1);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const [table] = await db.insert(kenoTables).values({
      slug,
      title,
