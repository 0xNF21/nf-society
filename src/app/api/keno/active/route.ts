...
import { kenoRounds, kenoTables } from "@/lib/db/schema";
import { eq, and, ne, desc } from "drizzle-orm";
import { getVisibleState } from "@/lib/keno";
import type { KenoState } from "@/lib/keno";

/**
 * GET /api/keno/active?tableSlug=classic&token=abc123
 *
...
export async function GET(req: NextRequest) {
  try {
    ...
    const [table] = await db.select().from(kenoTables)
      .where(eq(kenoTables.slug, tableSlug)).limit(1);
    if (!table) return NextResponse.json({ round: null });

    // Find the most recent playing round with this token
    const [round] = await db.select().from(kenoRounds)
      .where(
        and(
          eq(kenoRounds.tableId, table.id),
          eq(kenoRounds.playerToken, token),
          ne(kenoRounds.status, "finished"),
        )
      )
      .orderBy(desc(kenoRounds.createdAt))
      .limit(1);
