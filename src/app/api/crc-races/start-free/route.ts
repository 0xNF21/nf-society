...
export async function POST(req: NextRequest) {
  const limited = await enforceRateLimit(req, "crc-races-start-free", 20, 60000);
  if (limited) return limited;

  // Gate F2P — voir morpion/start-free pour le contexte audit PR1.
  if (await isRealStakesEnabled("crc-races")) {
  ...
  try {
    ...
    console.error("[crc-races/start-free] error:", err);
