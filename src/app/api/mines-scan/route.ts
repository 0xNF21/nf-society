...
import { minesTables, minesRounds, claimedPayments } from "@/lib/db/schema";
import { eq, inArray } from "drizzle-orm";
import { checkAllNewPayments } from "@/lib/circles";
import { createGrid, createInitialState } from "@/lib/mines";
import { awardPlayerXp } from "@/lib/xp-server";

const WEI_PER_CRC = BigInt("1000000000000000000");
const MINES_START_BLOCK = "0x2B7DE5C";

export async function POST(req: NextRequest) {
  const limited = await enforceRateLimit(req, "mines-scan", 10, 60000);
  if (limited) return limited;

  const disabled = await respondIfStakesDisabled("mines");
  ...
  try {
    ...
    const [table] = await db.select().from(minesTables).where(eq(minesTables.slug, tableSlug)).limit(1);
    if (!table) return NextResponse.json({ error: "Table not found" }, { status: 404 });

    const betOptions = (table.betOptions as number[]) || [1, 5, 10, 25];
    const mineOptions = (table.mineOptions as number[]) || [1, 3, 5, 10, 24];

    // Get existing rounds to skip known txs
    const existingRounds = await db
      .select({ transactionHash: minesRounds.transactionHash })
      .from(minesRounds)
      .where(eq(minesRounds.tableId, table.id));
    ...
    for (const bet of betOptions) {
      const payments = await checkAllNewPayments(bet, table.recipientAddress, MINES_START_BLOCK);
    ...
    for (const payment of allPayments) {
      ...
      if (payment.gameData.game !== "mines") continue;
      if (!payment.gameData.id.startsWith(table.slug)) continue;

      // Extract mine count from gameData.id: "{slug}-{mineCount}"
      const parts = payment.gameData.id.split("-");
      const mineCount = parseInt(parts[parts.length - 1], 10);
      if (isNaN(mineCount) || mineCount < 1 || mineCount > 24) continue;
      if (!mineOptions.includes(mineCount)) continue;
      ...
      const state = createInitialState(grid, betCrc);

      try {
        const inserted = await db.insert(minesRounds).values({
          tableId: table.id,
          playerAddress,
          ...
          status: state.status,
        }).onConflictDoNothing().returning({ id: minesRounds.id });
        ...
        {
          void awardPlayerXp({ address: playerAddress, action: "mines_play" }).catch(() => {});
        }

      } catch (err: any) {
        console.error("[MinesScan] Error:", err.message);
      }
    }

    return NextResponse.json({ created: created.length, results: created });
  } catch (error: any) {
    console.error("[MinesScan] Fatal:", error.message);
