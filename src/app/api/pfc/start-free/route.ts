...
export async function POST(req: NextRequest) {
  const limited = await enforceRateLimit(req, "pfc-start-free", 20, 60000);
  if (limited) return limited;

  // Gate F2P — voir morpion/start-free pour le contexte audit PR1.
  if (await isRealStakesEnabled("pfc")) {
  ...
  try {
    ...
    console.error("[pfc/start-free] error:", err);
