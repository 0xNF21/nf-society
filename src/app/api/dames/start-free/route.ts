...
export async function POST(req: NextRequest) {
  const limited = await enforceRateLimit(req, "dames-start-free", 20, 60000);
  if (limited) return limited;

  // Gate F2P — voir morpion/start-free pour le contexte audit PR1.
  if (await isRealStakesEnabled("dames")) {
  ...
  try {
    ...
    console.error("[dames/start-free] error:", err);
