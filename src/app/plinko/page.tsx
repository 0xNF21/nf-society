...
export const metadata: Metadata = {
  title: "Plinko — NF Society",
  description: "Lachez la bille, laissez la gravite decider.",
};

export default async function PlinkoLobbyPage() {
  const tables = await db
    .select()
    .from(plinkoTables)
    .where(eq(plinkoTables.status, "active"));

  return (
    <div className="min-h-screen px-4 py-8 max-w-2xl mx-auto">
      <Link href="/chance" className="inline-flex items-center gap-1.5 text-sm text-ink/40 hover:text-ink/60 transition-colors mb-6">
        &larr; Retour
      </Link>

      <div className="text-center mb-8">
        <div className="text-5xl mb-3">📌</div>
        <h1 className="text-3xl font-bold text-ink mb-2">Plinko</h1>
        <p className="text-sm text-ink/50 max-w-md mx-auto">
          Lachez la bille, laissez la gravite decider. RTP ~99%.
        </p>
      </div>
