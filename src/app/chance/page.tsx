      ...
      <div className="max-w-2xl mx-auto px-4 pt-16 pb-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {games.map((game, i) => {
            ...
            const className = `group relative rounded-2xl border-2 border-ink/5 bg-white/80 backdrop-blur-sm p-5 shadow-sm hover:shadow-lg ${game.borderHover} transition-all duration-300 flex flex-col items-center text-center gap-3`;

            if (game.type === "daily") {
              return (
                <button key={i} onClick={handleDailyClick} className={className}>
                  {inner}
                </button>
              );
            }
            ...
            return (
              <Link key={i} href={game.href!} className={className}>
                {inner}
              </Link>
            );
          })}
