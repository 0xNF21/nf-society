...
export default async function DiceLobbyPage() {
  const [tables, realStakesEnabled] = await Promise.all([
    db
    .select()
    .from(diceTables)
    .where(eq(diceTables.status, "active")),
    isRealStakesEnabled(),
  ]);
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-2xl mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-ink mb-2">Dice</h1>
        <p className="text-sm text-ink/50 max-w-md mx-auto">
          Lancez le de virtuel, choisissez votre cible. RTP ~99%.
        </p>
      </div>
