...
export const metadata: Metadata = {
  title: "Keno — NF Society",
  description: "Choisissez vos numeros, tirage aleatoire. RTP ~99%.",
};

export default async function KenoLobbyPage() {
  const tables = await db
    .select()
    .from(kenoTables)
    .where(eq(kenoTables.status, "active"));
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-2xl mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-ink mb-2">Keno</h1>
        <p className="text-sm text-ink/50 max-w-md mx-auto">
          Choisissez vos numeros, tirage aleatoire. RTP ~99%.
        </p>
      </div>
