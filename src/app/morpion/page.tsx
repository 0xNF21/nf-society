...
export default function MorpionLobby() {
  ...
  async function createGame() {
    ...
    try {
      ...
      // Mémoriser qu'on est le créateur (J1) de cette partie
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold text-ink mb-2">Morpion CRC</h1>
          <p className="text-ink/50 text-sm">Défiez un ami et misez des CRC on-chain</p>
        </div>

        {/* Create game */}
        <Card className="mb-4 bg-white/60 backdrop-blur-sm border-ink/10 shadow-sm rounded-2xl">
          <CardHeader className="pb-3">
            <CardTitle className="text-base text-ink">Créer une partie</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-ink/40 uppercase tracking-widest mb-2">Mise par joueur</label>
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                />
                <span className="text-sm text-ink/50 leading-none">CRC par joueur</span>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-ink/[0.03] border border-ink/5 text-xs text-ink/50">
              🏆 Le gagnant remporte <span className="font-bold text-ink">{Math.floor(betCrc * 2 * 0.95)} CRC</span>
              <span className="ml-1">(5% de commission)</span>
            </div>
            ...
            <Button onClick={createGame} disabled={loading} className="w-full rounded-xl font-bold"
              ...
              {loading ? "Création..." : `Créer la partie — ${betCrc} CRC`}
            </Button>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white/60 backdrop-blur-sm border-ink/10 shadow-sm rounded-2xl">
          <CardHeader className="pb-3">
            <CardTitle className="text-base text-ink">Rejoindre une partie</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-ink/40 uppercase tracking-widest mb-2">Code de la partie</label>
