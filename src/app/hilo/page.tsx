...
export const metadata: Metadata = {
  title: "Hi-Lo — NF Society",
  description: "Devinez si la prochaine carte est plus haute ou plus basse",
};

export default async function HiLoLobbyPage() {
  const [tables, realStakesEnabled] = await Promise.all([
    db
    .select()
    .from(hiloTables)
    .where(eq(hiloTables.status, "active")),
    isRealStakesEnabled(),
  ]);
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-2xl mx-auto">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-ink mb-2">Hi-Lo</h1>
        <p className="text-sm text-ink/50 max-w-md mx-auto">
          Devinez si la prochaine carte est plus haute ou plus basse. RTP ~97%.
        </p>
      </div>
